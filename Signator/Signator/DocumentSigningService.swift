...
public struct DocumentSigningService {
    ...
    struct UploadResponse: Decodable {
        ...
        let documentHash: String?  // Optional - may not be returned by newer server versions
        let ledgerProofEntryID: String?  // Optional - only present if ledger service is available
        let ledgerProofIndex: String?  // Optional - only present if ledger is enabled
        let accessCode: String?  // Human-readable access code for private documents
    ...
    struct SignatureResponse: Decodable {
        let entryID: String  // Signature entry ID from signatures file
        let signerDID: String
        let role: String
        let documentId: String
        let success: Bool
        let message: String
        let timestamp: String?  // Optional - may not be returned by server
        
        // Legacy fields (no longer used by server, but kept for backward compatibility)
        var ledgerIndex: Int? { nil }  // Always nil - ledger system deprecated
